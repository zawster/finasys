...
class TestSchemaStandardization:
    ...
    def test_yahoo_column_names(self):
        ...
        df = pl.DataFrame({"Date": ["x"], "Open": [1.0], "High": [2.0],
...
class TestCache:
    ...
    def test_cache_put_and_get(self, cache_df):
        from alphakit.sources.cache import cache_put, cache_get, cache_clear
        cache_clear("CTEST")
    ...
    def test_cache_get_empty(self):
        from alphakit.sources.cache import cache_get, cache_clear
...
class TestSMA:
    ...
    def test_correctness(self):
        from alphakit.features import sma
        df = pl.DataFrame({"timestamp": [date(2024,1,i) for i in range(1,6)],
...
class TestROC:
    ...
    def test_correctness(self):
        from alphakit.features import roc
        df = pl.DataFrame({"timestamp": [date(2024,1,i) for i in range(1,4)],
...
class TestReturns:
    ...
    def test_correctness(self):
        from alphakit.features import returns
        df = pl.DataFrame({"timestamp": [date(2024,1,1), date(2024,1,2)],
                           "close": [100.0, 105.0]})
...
class TestRollingStats:
    ...
    def test_all_stats(self, sample_ohlcv):
        from alphakit.features import rolling_stats
        r = rolling_stats(sample_ohlcv, windows=10,
                          stats=["mean", "std", "min", "max", "skew", "zscore"])
...
class TestAddAll:
    ...
    def test_everything_on(self, sample_ohlcv):
        from alphakit.features import add_all
        r = add_all(sample_ohlcv, indicators=True, returns_=True,
                     lags_=[1,5], rolling_windows=[5,21], calendar=True)
        assert r.width > 30
...
class TestFeatureSet:
    def test_create_and_transform(self, sample_ohlcv):
        from alphakit.features import FeatureSet, RSI, MACD
    ...
    def test_add_chaining(self, sample_ohlcv):
        from alphakit.features import FeatureSet, RSI, Returns
        fs = FeatureSet()
    ...
    def test_all_step_types(self, sample_ohlcv):
        ...
        from alphakit.features import (
            FeatureSet, RSI, MACD, BollingerBands, ATR, Returns,
            LogReturns, RollingStats, Lags, Calendar,
        )
        fs = FeatureSet([
            RSI(period=14),
    ...
    def test_roundtrip_produces_identical_output(self, sample_ohlcv, tmp_path):
        from alphakit.features import FeatureSet, RSI, MACD, Returns
    ...
    def test_repr(self):
        from alphakit.features import FeatureSet, RSI
...
class TestTools:
    ...
    def test_tool_names(self):
        ...
        assert names == {"lookup_price", "get_technical_indicators",
                         "compare_symbols", "get_summary"}
